...
async def run(robot: cozmo.robot.Robot):
    ...
    while True:
        ...
        if localize_converge:
            ...
            if goal_reached:
                # play the animation sound for happiness(amazed)
                ...
                await robot.turn_in_place(cozmo.util.degrees(diff_heading_deg(goal[2], z + (diff_heading_deg(math.degrees(math.atan2(goal[1] - y, goal[0] - x)), z)))),
                                    speed=cozmo.util.degrees(90)).wait_for_completed()
                goal_reached = True
        else:
            mkr_list, img = await marker_processing(robot, camera_settings)
            ps = robot.pose
            # update the coordinates and the convergence status
            x, y, z, localize_converge = pf.update(compute_odometry(ps), mkr_list)
            gui.show_particles(pf.particles)
            gui.show_mean(x, y, z, localize_converge)
            gui.show_camera_image(img)
            gui.updated.set()
            last_pose = ps
            counter += 1
            if not localize_converge:
                # perform the corresponding action
                # get the motion count for motion index
                if counter % 48 <= 24:
                    await robot.turn_in_place(cozmo.util.degrees(-30)).wait_for_completed()
                else:
                    # rotate backwards after rotating clockwise for one cycle, in order to
