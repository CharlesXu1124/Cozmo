...
class CozMap:
    ...
    def compute_smooth_path(self, limit=75):
        ...
        path = self.get_path()
        # loop through the path list and sample a random list of adjacent paths (used for smoothing)
        for i in range(20 * len(path)):
            [node0, node1] = [path[j] for j in sorted(random.sample(range(len(path)), 2))]
            # if the node0 to node1 is obstructed by an obstacle
            # omit that edge
